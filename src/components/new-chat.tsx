...
export function NewChat() {
  ...
  // ========================
  const getMessageContent = useCallback((message: unknown) => {
    if (message && typeof message === 'object' && 'parts' in message) {
      const msgObj = message as { parts?: unknown };
  ...
  const loadChatHistory = useCallback(async (sessionIdToLoad: string, signal: AbortSignal) => {
    ...
    try {
      ...
      const formattedMessages = loadedMessagesData.map((msg: { id?: string; role?: string; content?: string; text?: string }, index: number) => ({
